namespace :db do
  ...
  task :populate => :environment do
    ...
    CSV.foreach(‘restaurants.csv’) do |row|
        name = row[0]
        street = row[1]
        zip_code = row[2].to_i
        cuisine = row[3]
        ...
        latitude = row[6].to_f
        accepts_dinex = row[7].to_bool
        accepts_cash = row[8].to_bool
        accepts_credit = row[9] ].to_bool
        accepts_plaid_cash = row[10] ].to_bool
        price = row[11].to_i
        waiting_time = row[12].to_i
        Restaurant.create(name: name, street: street, city: "Pittsburgh", state: "PA", zip_code: zip_code, cuisine: cuisine, area: area, longitude: longitude, latitude: latitude, accepts_dinex: accepts_dinex, accepts_cash: accepts_cash, accepts_credit: accepts_credit, accepts_plaid_cash: accepts_plaid_cash, price: price, waiting_time: waiting_time)
    end
